...
void test_version_and_null_contract() {
    ...
    uint32_t quality_version = 0;
    uint32_t quality_size = 0;
    CHECK(CapQualityGetVersion(&quality_version, &quality_size) == S_OK);
    CHECK(quality_version == kCaptureQualityNativeVersion);
    CHECK(quality_size == sizeof(CaptureQualityNative));
    CHECK(CapQualityGetVersion(nullptr, &quality_size) == E_POINTER);
    CHECK(CapQualityGetVersion(&quality_version, nullptr) == E_POINTER);
    CaptureQualityNative quality{};
    quality.structSize = sizeof(quality);
    quality.version = kCaptureQualityNativeVersion;
    CHECK(CaptureConfigureQuality(0, 1) == E_HANDLE);
    CHECK(CaptureGetQualityResult(0, &quality) == E_HANDLE);
    CHECK(CaptureGetQualityResult(0, nullptr) == E_POINTER);
