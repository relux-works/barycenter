...
- Media authorization is a dedicated owner/immutable-target-snapshot service.
  ...
  exposes a canonical cancellation interface before transmission consumes it;
  cross-service cancellation is proven in Phase 1 acceptance rather than by a
  reverse ingest-to-transmission story dependency.
...
- Policy/publication/moderation foundations live in
  `STORY-260712-1tgryz`; platform and Telegram integrations plus screenshots,
  independent reviews and Partner Center submission remain in
  `STORY-260712-1i0doc`. This makes the order foundation → Telegram/UI → Store
  acceptance explicit and removes the former compliance↔Telegram cycle.
