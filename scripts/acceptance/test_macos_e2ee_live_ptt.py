...
SPEC = importlib.util.spec_from_file_location(
    "macos_e2ee_live_ptt", HERE / "validate_macos_e2ee_live_ptt.py",
)
...
class MacE2EELivePTTAcceptanceTests(unittest.TestCase):
    ...
    def test_production_enablement_fails_closed(self):
        ...
        with self.assertRaisesRegex(MODULE.MacE2EELivePTTError, "production-dark"):
    ...
    def test_bounds_drift_fails_closed(self):
        ...
        with self.assertRaisesRegex(MODULE.MacE2EELivePTTError, "bounds"):
            MODULE.validate(contract)

    def test_manual_evidence_fails_closed(self):
        contract = MODULE.load()
        contract["manualEvidence"]["hardwareC1C2Latency"] = "passed"
        with self.assertRaisesRegex(MODULE.MacE2EELivePTTError, "manual evidence"):
            MODULE.validate(contract)
