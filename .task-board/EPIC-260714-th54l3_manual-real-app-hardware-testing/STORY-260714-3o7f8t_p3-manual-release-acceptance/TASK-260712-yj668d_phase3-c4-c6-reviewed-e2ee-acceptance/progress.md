## Status
closed

## Assigned To
(none)

## Created
2026-07-12T16:41:36Z

## Last Update
2026-07-21T10:59:27Z

## Blocked By
- TASK-260712-3da0vz
- TASK-260712-2uo81g
- TASK-260712-1bcpda
- TASK-260712-3g0axs
- TASK-260712-1ulshp
- TASK-260712-7ng1vs
- TASK-260712-flaiie

## Blocks
- TASK-260712-30xwu2
- TASK-260712-1gyohk

## Checklist
- [ ] Run membership join/leave/revoke and history-grant scenarios against the final build
- [ ] Capture storage and traffic evidence proving coordinator non-decryption
- [ ] Verify secure-storage and log-redaction invariants on both node platforms
- [ ] Exercise voluntary evidence-copy reporting and moderation workflow end to end
- [ ] Publish a dated C4-C6 pack with exact flag posture and residual blockers

## Notes
2026-07-14 scope routing: moved to EPIC-260714-th54l3 Manual real-app hardware testing. This task is deferred for hands-on execution in original sequence and no longer gates best-effort coding, unit tests or deterministic CI in EPIC-260712-3agrc1.
2026-07-21 owner-directed consolidation: closed as superseded, not passed. Remaining highest-value real-app and hardware observations now live only in TASK-260721-ryk8c0 Ivan Oparin final real-app verification, gated by TASK-260721-2346wf Desktop UI automated acceptance and owner handoff.

## Precondition Resources
- [p3-acceptance-evidence-map.puml](file://TASK-260712-yj668d/p3-acceptance-evidence-map.puml) — Upstream evidence and disclosure context for C4-C6 acceptance

## Outcome Resources
(none)
