...
spec = importlib.util.spec_from_file_location(
    "p3_privacy_store_pre_review", HERE / "validate_p3_privacy_store_pre_review.py"
)
...
class Phase3PrivacyStorePreReviewContractTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_rejects_false_independence_submission_or_activation(self):
        ...
        for key in ("storeSubmissionAllowed", "affectedPhase3FlagsAllowed", "phase3PromotionAllowed"):
            changed = copy.deepcopy(self.data)
            changed["decision"][key] = True
            with self.assertRaisesRegex(contract.ReviewError, "fail-closed"):
                contract.validate(changed)

    def test_rejects_external_publication_mail_or_portal_claim(self):
        for key in ("policyPublicationClaimed", "mailboxDeliveryClaimed", "partnerCenterEvidenceClaimed"):
            changed = copy.deepcopy(self.data)
            changed["decision"][key] = True
            with self.assertRaisesRegex(contract.ReviewError, "fail-closed"):
                contract.validate(changed)

    def test_rejects_source_or_claim_drift(self):
        changed = copy.deepcopy(self.data)
        changed["sourceAnchors"][0]["sha256"] = "0" * 64
        with self.assertRaisesRegex(contract.ReviewError, "source digest mismatch"):
            contract.validate(changed)
        changed = copy.deepcopy(self.data)
        changed["reviewedClaims"][-1]["result"] = "match"
        with self.assertRaisesRegex(contract.ReviewError, "external Store claim was not held"):
            contract.validate(changed)
    ...
    def test_rejects_removed_external_gate_or_open_finding(self):
        ...
        with self.assertRaisesRegex(contract.ReviewError, "open gate inventory"):
            contract.validate(changed)
        changed = copy.deepcopy(self.data)
        changed["findings"] = [{"id": "P3-PRIV-001", "severity": "high", "status": "open"}]
