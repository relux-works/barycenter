...
| **F2** Pairing survives | Login Keychain + migration; legacy yml auto-retire | **FIXED; RELEASE PROOF REQUIRED** | `NodeCore/Keychain.swift` avoids the entitlement-gated DP Keychain and is covered through an injected Security.framework boundary; `KeychainTests.swift` pins the no-DP query and re-pair repair; `Config.swift` retire + test. Re-run a signed Sparkle update after release. |
