...
## Checklist
- [x] Implement session creation and scoped upload authorization endpoints
- [x] Support monotonic resume offsets, idempotency keys and phase-one quota enforcement
- [x] Cover retry, unauthorized access, malformed requests and concurrent limit behavior with tests
- [x] Use expiring scoped tokens, actual-byte enforcement and concurrency-safe monotonic offsets
- [x] Test restart, repeated finalize and abandoned-session cleanup
...
Hosted CI run 29300399021 passed coordinator, node-core, pulsar-win and signed packaged-probe on commit b1b7576. Root delta review added scheduled expiry, orphan staging cleanup, forced private permissions, concurrent quota reservation and exact immediate-predecessor rollback coverage; no unresolved coding or automated-verification finding remains.
