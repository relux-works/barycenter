...
func openWithOptionsAndCheckpoint(path string, opts Options, checkpoint func(string) error) (*Store, error) {
	...
	if _, err := s.ReconcileE2EEOpaqueLiveRestart(time.Now().UnixMilli()); err != nil {
		db.Close()
		return nil, fmt.Errorf("store: reconcile dormant E2EE live sessions: %w", err)
	}
	...
	if err := s.initAutomationControlSchema(); err != nil {
		db.Close()
		return nil, fmt.Errorf("store: init automation control schema: %w", err)
	}
	if err := s.initAutomationRuntimeSchema(); err != nil {
		db.Close()
