...
7. **Picker access cannot end at a filesystem path.**
   A brokered `StorageFile` can represent a provider-backed/virtual item, and
   returning only `StorageFile.Path` does not freeze a legal readable-data path
   for the Go side. Select and document either WinRT stream reads inside the
   helper or `IStorageItemHandleAccess::Create` with an explicit same-process
   HANDLE ownership/close contract. Cover cancel, zero-byte/read failure, cloud
   hydration, and non-filesystem items without adding broad capabilities. The
   probe must prove it can read bytes, not merely display a picked name/path.
