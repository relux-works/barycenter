...
SPEC = importlib.util.spec_from_file_location(
    "macos_protected_media_playback",
    HERE / "validate_macos_protected_media_playback.py",
)
...
class MacProtectedMediaPlaybackAcceptanceTests(unittest.TestCase):
    ...
    def test_production_enablement_fails_closed(self):
        ...
        with self.assertRaisesRegex(MODULE.MacProtectedMediaPlaybackError, "production-dark"):
    ...
    def test_bounds_drift_fails_closed(self):
        ...
        with self.assertRaisesRegex(MODULE.MacProtectedMediaPlaybackError, "bounds"):
            MODULE.validate(contract)

    def test_manual_evidence_fails_closed(self):
        contract = MODULE.load()
        contract["manualEvidence"]["hardwarePlaybackSeekInterrupt"] = "passed"
        with self.assertRaisesRegex(MODULE.MacProtectedMediaPlaybackError, "manual evidence"):
            MODULE.validate(contract)
