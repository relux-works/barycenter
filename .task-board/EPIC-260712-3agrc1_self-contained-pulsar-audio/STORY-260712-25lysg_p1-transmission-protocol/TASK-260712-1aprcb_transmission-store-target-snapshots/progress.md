## Status
development

## Assigned To
codex-inline

## Created
2026-07-12T15:39:19Z

## Last Update
2026-07-14T07:20:55Z

## Blocked By
- TASK-260712-51y5k9
- TASK-260712-3mcof4
- TASK-260712-z6h6wh

## Blocks
- TASK-260712-2qpp6w
- TASK-260712-31vvjt
- TASK-260712-2qc27p
- TASK-260712-2hcq1g
- TASK-260712-3lf8r0
- TASK-260712-2h6snp
- TASK-260712-2bk0vy

## Checklist
- [ ] Add additive schema for transmissions, target snapshots, receipts, and block data
- [ ] Persist per-target state transitions and ordering fields
- [ ] Replace media GET auth with transmission target ACL and cover migration plus rollback tests
- [ ] Persist expiry, effective delivery and immutable offline plus online target snapshots
- [ ] Integrate the generic ACL service without authorizing from current membership

## Notes
Strict sequential handoff after accepted TASK-260712-51y5k9. Start only from merged PR #20/main; implement the frozen p1-transmission-v1 persistence and immutable ACL contract. Manual real-app and physical-hardware evidence remains in EPIC-260714-th54l3.

## Precondition Resources
- [p1-transmission-protocol-components.puml](file://TASK-260712-1aprcb/p1-transmission-protocol-components.puml) — Store and ACL context for transmission persistence

## Outcome Resources
(none)
