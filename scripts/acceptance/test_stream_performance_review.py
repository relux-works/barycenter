...
def load_module():
    spec = importlib.util.spec_from_file_location(
        "stream_performance_review", HERE / "validate_stream_performance_review.py"
    )
...
class StreamPerformanceReviewTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_current_review_is_valid_and_fail_closed(self):
        ...
        self.assertFalse(self.review["decision"]["productionPlaybackAllowed"])
    ...
    def test_rejects_production_or_manual_claim_escape(self):
        changed = copy.deepcopy(self.review)
        changed["decision"]["productionPlaybackAllowed"] = True
        with self.assertRaisesRegex(review_contract.ReviewError, "production playback"):
            review_contract.validate(changed)

        changed = copy.deepcopy(self.review)
        changed["decision"]["manualPerformanceClaim"] = True
        with self.assertRaisesRegex(review_contract.ReviewError, "claimed manual performance"):
            review_contract.validate(changed)

    def test_rejects_threshold_or_pairing_drift(self):
        changed = copy.deepcopy(self.review)
        changed["thresholds"]["trackStartP95MS"] = 5001
        with self.assertRaisesRegex(review_contract.ReviewError, "threshold drifted"):
            review_contract.validate(changed)

        changed = copy.deepcopy(self.review)
        changed["thresholds"]["requiredPairings"] = ["windows_windows"]
        with self.assertRaisesRegex(review_contract.ReviewError, "pairing matrix"):
            review_contract.validate(changed)

    def test_rejects_silent_high_finding_closure(self):
        changed = copy.deepcopy(self.review)
        changed["findings"][1]["status"] = "fixed-re-reviewed"
