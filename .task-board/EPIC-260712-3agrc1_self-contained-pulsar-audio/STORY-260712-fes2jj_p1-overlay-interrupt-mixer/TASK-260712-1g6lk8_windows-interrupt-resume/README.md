# Implement Windows interrupt with audible-position resume
...
## Acceptance Criteria
A4 holds on Windows: the Spotify main program resumes within 500 ms of the preserved audible position without duplicate tail, missing samples or a second resume. Unsupported capability is explicit and never silently becomes overlay or after_current. Cancel and reconnect leave one terminal clip state, no armed timer and no stale daemon command.
