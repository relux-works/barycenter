...
- Громкость: требование |ΔLU| между домами <= 2. [РЕШЕНИЕ] (a) гейн из Track.r128 к эквиваленту -14 LUFS как основной путь; фолбэк при отсутствии r128 — [ВАРИАНТЫ] (b) бандлить ffmpeg в Pulsar.app (+25..70МБ к 6МБ dmg, чужой бинарь в цепочке подпись/нотаризация/attestation — дорого); (c) [АРХ-предложение] замер на координаторе: у Barycenter ffmpeg уже есть (docker-образ); координатор скачивает файл, меряет loudnorm и кладёт готовый гейн-СКАЛЯР в load-payload (поле gain_db?) — нода применяет число, dmg не толстеет, «аудио между домами не ходит» цел (летит одно число); цена — координатору нужен доступ к download_info (тот же §0-вопрос). Решение после spike-2/S5; предварительный фаворит (c).
