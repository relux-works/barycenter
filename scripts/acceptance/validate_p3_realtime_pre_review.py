#!/usr/bin/env python3
"""Fail-closed validation for the Phase 3 realtime technical pre-review."""
...
REVIEW_PATH = ROOT / "acceptance/phase3/realtime-technical-pre-review-v1.json"
...
def validate(review: dict) -> None:
    ...
    require(review.get("contract") == "p3-realtime-technical-pre-review.v1", "wrong contract")
    require(review.get("task") == "TASK-260712-3j4a06", "wrong task")
    ...
    require(set(root.get("findingStatus", {})) == {
        "P3-ROOT-001", "P3-ROOT-002", "P3-ROOT-003", "P3-ROOT-004",
    }, "root finding inventory drifted")
    require(set(root["findingStatus"].values()) == {"fixed-re-reviewed"},
            "root finding reopened")
    root_packet = json.loads(ROOT_REVIEW_PATH.read_text(encoding="utf-8"))
    require(root_packet.get("reviewedSourceCommit") == root["sourceCommit"],
            "root packet source mismatch")
    require(root_packet.get("reviewedSourceTree") == root["sourceTree"],
            "root packet tree mismatch")
    ...
    require(reviewer.get("independentApprovalTask") == "TASK-260717-3dbi2v",
    ...
    for key in ("livePTTActivationAllowed", "c1ThroughC3Accepted", "phase3PromotionAllowed", "manualEvidenceClaimed"):
    ...
    require(len(anchors) == 25, "source inventory incomplete")
    ...
    require(set(review.get("reviewedInvariants", [])) == {
        "one-generation-one-capture-owner",
        "lost-release-watchdog-and-terminal-cleanup",
        "bounded-coordinator-session-and-target-state",
        "bounded-jitter-window-with-fec-before-plc",
        "slow-target-backpressure-does-not-stall-fast-targets",
        "callback-paths-have-no-network-storage-lock-or-allocation",
        "audible-start-follows-render-consumption",
        "post-mix-limiter-and-input-ceiling-cannot-be-bypassed",
        "route-or-processing-ambiguity-degrades-fail-closed",
    }, "review invariant inventory drifted")

    reruns = review.get("reruns", [])
    require({item.get("id") for item in reruns} == {
        "coordinator-live-race-stress", "windows-live-capture-race-stress",
        "macos-live-capture-suite", "transport-capture-contracts",
    ...
    require(len(attempts) == 1 and
            attempts[0].get("id") == "coordinator-unrelated-full-store-race-times-ten" and
            attempts[0].get("status") == "timeout-not-counted" and
            "unrelated identity rollback" in attempts[0].get("reason", ""),
    ...
    require(matrix.get("gates", {}).get("C1", {}).get("status") == "manual-required",
            "C1 no longer manual-required")
    require(matrix.get("gates", {}).get("NF-realtime-review", {}).get("status") ==
            "missing-independent-reviewer", "realtime independent gate drifted")
    require(review.get("nextEngineeringTask") == "TASK-260712-1x5jfo", "strict next task drifted")
