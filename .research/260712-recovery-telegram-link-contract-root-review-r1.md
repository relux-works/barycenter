...
2. **Recovery can permanently lock a user out after a lost HTTP response.**
   ...
   Define retry and race behavior explicitly. If the client-generated pattern is
   selected, the candidate must be put into protected pending credential storage
   before the destructive request; keeping it only in RAM still loses access if
   the app or machine dies after server commit. Define how a retry or an
   authenticated probe promotes/discards that pending credential without ever
   reviving an older control token.
