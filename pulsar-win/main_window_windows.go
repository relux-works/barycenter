...
const (
	...
	gwlStyle = ^uintptr(15) // GWL_STYLE (-16), sign-extended for x64.
)
...
type mainWindowCtx struct {
	...
	all                       []windows.Handle
	controlBounds             map[windows.Handle]ShellRect
	appliedBounds             map[windows.Handle]ShellRect
	fonts                     mainFonts
...
func (ctx *mainWindowCtx) createControls() {
	...
	for index, section := range shellSections {
		style := uint32(buttonStyle | bsFlat | bsMultiline)
		if index == 0 {
			style |= wsGroup
		}
		ctx.nav[section] = mk(0, "BUTTON", "", style, shellSectionControlID(section))
	}
	ctx.title = mk(0, "STATIC", "", wsChild|wsVisible|ssLeft, 0)
	ctx.banner = mk(0, "STATIC", "", wsChild|wsVisible|ssLeft, 0)
	ctx.body = mk(0, "STATIC", "", wsChild|wsVisible|ssLeft, 0)
	ctx.home[0] = mk(0, "BUTTON", "", buttonStyle|bsDefButton|bsMultiline, idShellCreate)
	...
	for i := range ctx.cards {
		ctx.cards[i] = mk(0, "STATIC", "", wsChild|wsVisible|ssLeft, 0)
	}
	ctx.footer = mk(0, "STATIC", "", wsChild|wsVisible|ssLeft, 0)
	ctx.detail = mk(0, "BUTTON", "", buttonStyle|bsDefButton|bsMultiline, idShellAction)
...
func (ctx *mainWindowCtx) updateFonts() {
	...
	ctx.fonts = mainFonts{
		dpi:   dpi,
		title: mkFont(-dip(22, dpi), fwSemibold),
		body:  mkFont(-dip(14, dpi), fwNormal),
		small: mkFont(-dip(12, dpi), fwNormal),
	}
...
func move(control windows.Handle, rect ShellRect) {
	if control == 0 || rect.Width <= 0 || rect.Height <= 0 {
		return
	}
	if mainCtx == nil {
		pMoveWindow.Call(uintptr(control), uintptr(rect.X), uintptr(rect.Y), uintptr(rect.Width), uintptr(rect.Height), 0)
		return
	}
	if mainCtx.controlBounds == nil {
		mainCtx.controlBounds = make(map[windows.Handle]ShellRect)
		mainCtx.appliedBounds = make(map[windows.Handle]ShellRect)
	}
	mainCtx.controlBounds[control] = rect
	style, _, _ := pGetWindowLongW.Call(uintptr(control), gwlStyle)
	if style&wsVisible == 0 {
		// Inactive pages still receive their desired layout, but moving their
		// HWNDs would invalidate the parent and make navigation needlessly slow.
		return
	}
	if previous, ok := mainCtx.appliedBounds[control]; ok && previous == rect {
		return
	}
	repaint := uintptr(0)
	if !mainCtx.rendering {
		repaint = 1
	}
	pMoveWindow.Call(uintptr(control), uintptr(rect.X), uintptr(rect.Y), uintptr(rect.Width), uintptr(rect.Height), repaint)
	mainCtx.appliedBounds[control] = rect
}
...
func showControl(control windows.Handle, visible bool) {
	...
	style, _, _ := pGetWindowLongW.Call(uintptr(control), gwlStyle)
	if (style&wsVisible != 0) == visible {
		return
	}
	command := uintptr(swHide)
	if visible {
		if mainCtx != nil {
			if rect, ok := mainCtx.controlBounds[control]; ok {
				if previous, applied := mainCtx.appliedBounds[control]; !applied || previous != rect {
					pMoveWindow.Call(uintptr(control), uintptr(rect.X), uintptr(rect.Y), uintptr(rect.Width), uintptr(rect.Height), 0)
					mainCtx.appliedBounds[control] = rect
				}
			}
...
func (ctx *mainWindowCtx) hidePageControls() {
	// Navigation and the shell header are structural chrome. Keeping their
	// HWNDs visible across a page transition avoids blank sidebar/header frames
	// while the active page controls are swapped.
	persistent := map[windows.Handle]bool{
		ctx.title: true, ctx.banner: true, ctx.body: true, ctx.dnd: true,
	}
	for _, control := range ctx.nav {
		persistent[control] = true
	}
	for _, control := range ctx.all {
		if !persistent[control] {
			showControl(control, false)
		}
	}
}

func (ctx *mainWindowCtx) repaintStructuralChrome() {
	controls := []windows.Handle{ctx.title, ctx.banner, ctx.body, ctx.record, ctx.captureQualityStop, ctx.dnd}
	for _, control := range ctx.nav {
...
func (ctx *mainWindowCtx) layout() {
	...
	if ctx.shell != nil && (ctx.shell.Section() == ShellTryLocally || ctx.shell.Section() == ShellSettings) {
		layout.Body.Height = dip(260, layout.DPI)
	}
	if ctx.shell != nil && ctx.shell.Section() == ShellAirs {
		layout.Body.Height = dip(200, layout.DPI)
	}
	if ctx.shell != nil && ctx.shell.Section() == ShellInbox {
		layout.Body.Height = dip(220, layout.DPI)
	}
	if ctx.shell != nil && ctx.shell.Section() == ShellHistory {
		layout.Body.Height = dip(160, layout.DPI)
	}
	...
	move(ctx.banner, ShellRect{X: layout.Banner.X + bannerPad, Y: layout.Banner.Y + bannerPad, Width: layout.Banner.Width - bannerPad*2, Height: layout.Banner.Height - bannerPad*2})
	move(ctx.body, layout.Body)
	for index := range ctx.home {
		move(ctx.home[index], ShellRect{X: layout.Body.X + index*(layout.Body.Width-gap*2)/3 + index*gap, Y: layout.Body.Y + dip(76, layout.DPI), Width: (layout.Body.Width - gap*2) / 3, Height: dip(44, layout.DPI)})
	...
	reportY := layout.Body.Bottom() + gap + dip(144, layout.DPI)
	...
	trackY := reportY + dip(44, layout.DPI)
	...
	if !ctx.rendering {
		...
		pUpdateWindow.Call(uintptr(ctx.hwnd))
	}
}

func (ctx *mainWindowCtx) render() {
	if ctx.shell == nil || ctx.rendering {
		return
	}
	// Native callbacks and accessibility providers can re-enter the owner
	// thread while controls are being updated. A nested full render used to
	// multiply that work and could strand the packaged app outside its pump.
	ctx.rendering = true
	defer func() { ctx.rendering = false }()
	...
	if sectionChanged {
		...
		// messages. Keep structural chrome alive, swap only the page controls, and
		// invalidate the clipped parent background. A recursive RDW_ALLCHILDREN
		// erase blanked already-correct buttons until a later paint on Win10.
		defer func() {
			pInvalidateRect.Call(uintptr(ctx.hwnd), 0, 1)
			pUpdateWindow.Call(uintptr(ctx.hwnd))
			ctx.repaintStructuralChrome()
		}()
		ctx.hidePageControls()
	...
	banner := copy.Text(txtStatus) + ": " + copy.Connection(snapshot)
	...
	if section == ShellCreate || section == ShellJoin || section == ShellTryLocally || section == ShellSettings {
		...
		ctx.renderSectionBody(copy, snapshot, section)
		return
	}
	if section == ShellHistory || section == ShellSoundboard {
		...
		showTrackWorkflow := historyPage && (track.State == TargetsInboxReady || track.Draft != nil)
		for _, control := range trackControls {
			showControl(control, historyPage && (showTrackWorkflow || control == ctx.trackFile || control == ctx.trackRefresh))
		}
	...
	if section == ShellInbox {
		...
		for _, control := range targetControls {
			showControl(control, ready || control == ctx.targetsRefresh)
		}
		selectedAudience := "—"
	...
	if section == ShellAutomation {
		...
		for _, control := range automationControls {
			showControl(control, automation.Available || control == ctx.automationRefresh)
		}
		...
		showControl(ctx.automationConfirm, automationReady && automation.ConfirmAction != "")
		showControl(ctx.automationCancel, automationReady && automation.ConfirmAction != "")
	...
	// Shell.Select rejects unknown sections, but keep the body honest if a
	// future section is introduced before its native surface is wired.
	ctx.renderSectionBody(copy, snapshot, section)
}

func (ctx *mainWindowCtx) renderSectionBody(copy ShellCopy, snapshot ShellSnapshot, section ShellSection) {
...
func mainWindowProc(hwnd windows.Handle, message uint32, wParam, lParam uintptr) (result uintptr) {
	defer func() {
		if recovered := recover(); recovered != nil {
			if curTray != nil && curTray.Log != nil {
				curTray.Log.Error("main window callback recovered",
					"message", message, "panic", recovered, "stack", string(debug.Stack()))
			}
			// Keep the shell alive and let the next timer project a normalized
			// snapshot. A single malformed action must not tear down AppContainer.
			result = 0
		}
	}()
	ctx := mainCtx
