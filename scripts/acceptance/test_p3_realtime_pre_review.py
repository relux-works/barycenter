...
spec = importlib.util.spec_from_file_location(
    "p3_realtime_pre_review", HERE / "validate_p3_realtime_pre_review.py"
)
...
class Phase3RealtimePreReviewContractTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_rejects_false_independence_or_activation(self):
        ...
        for key in ("livePTTActivationAllowed", "c1ThroughC3Accepted", "phase3PromotionAllowed"):
            changed = copy.deepcopy(self.data)
            changed["decision"][key] = True
            with self.assertRaisesRegex(contract.ReviewError, "fail-closed"):
                contract.validate(changed)

    def test_rejects_manual_hardware_claim_or_source_drift(self):
        changed = copy.deepcopy(self.data)
    ...
    def test_rejects_silently_removed_external_gate_or_open_finding(self):
        changed = copy.deepcopy(self.data)
        changed["openGates"] = changed["openGates"][:1]
        with self.assertRaisesRegex(contract.ReviewError, "open gate inventory"):
            contract.validate(changed)
        changed = copy.deepcopy(self.data)
        changed["findings"] = [{"id": "P3-RT-001", "severity": "high", "status": "open"}]
